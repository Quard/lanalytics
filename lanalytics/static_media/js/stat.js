...
var timezone=determine_timezone().timezone;LAnalytics.config={'path':location.pathname,'browser':BrowserDetect.browser,'browser_version':BrowserDetect.version,'platform':BrowserDetect.OS,'screen_resolution':window.screen.width+'x'+window.screen.height,'window_dimensions':windowWidth+'x'+windowHeight,'enabled_cookie':areCookiesEnabled()?'1':'','flash':LAnalytics.haveFlashTrigger(),'have_java':navigator.javaEnabled(),'referrer':document.referrer,'time_zone':timezone.utc_offset,'api_key':key,'callback':LAnalytics.CALLBACK_FUNC};LAnalytics.stat(LAnalytics.config);},'haveFlashTrigger':function(){var flashVer=swfobject.getFlashPlayerVersion();if(flashVer&&flashVer.minor&&flashVer.major&&flashVer.release){return flashVer.major+'.'+flashVer.minor+'.'+flashVer.release;}
